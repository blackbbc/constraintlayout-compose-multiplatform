 ...
 */
package androidx.constraintlayout.core.shaded.utils
...
class GridEngine {
    ...
    private fun parseSpans(str: String): Array<IntArray>? {
        ...
        for (i in spans.indices) {
            indexAndSpan =
                spans[i].trim { it <= ' ' }.split(":".toRegex()).dropLastWhile { it.isEmpty() }
                    .toTypedArray()
            rowAndCol =
                indexAndSpan[1].split("x".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
    ...
    fun setSpans(spans: CharSequence) {
        if (mStrSpans != null && mStrSpans == spans.toString()) {
            return
        }
