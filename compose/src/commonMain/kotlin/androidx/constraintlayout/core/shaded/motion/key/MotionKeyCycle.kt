...
class MotionKeyCycle : MotionKey() {
    ...
    fun addCycleValues(oscSet: HashMap<String, KeyCycleOscillator>) {
        for (key in oscSet.keys) {
            if (key.startsWith(TypedValues.S_CUSTOM)) {
                val customKey = key.substring(TypedValues.S_CUSTOM.length + 1)
                val cValue = mCustom!![customKey]
                if (cValue == null || cValue.getType() != TypedValues.Custom.TYPE_FLOAT) {
                    continue
                }
    ...
    fun printAttributes() {
        ...
        val names = nameSet.toTypedArray<String?>()
