...
class MotionPaths : Comparable<MotionPaths> {
    ...
    var mCustomAttributes = HashMap<String, CustomVariable>()
    var mMode = 0 // how was this point computed 1=perpendicular 2=deltaRelative

    var mAnimateCircleAngleTo = 0 // since angles loop there are 4 ways we can pic direction
    ...
    fun initCartesian(c: MotionKeyPosition, startTimePoint: MotionPaths, endTimePoint: MotionPaths) {
        val position = c.mFramePosition / 100f
        val point = this
    ...
    fun initPolar(
        ...
        when (c.mPositionType) {
            MotionKeyPosition.TYPE_SCREEN -> {
                mX = if (c.mPercentX.isNaN()) {
                    position * (e.mX - s.mX) + s.mX
                } else {
                    (
                        c.mPercentX * min(
                            scaleHeight.toDouble(),
                            scaleWidth.toDouble(),
                        )
                        ).toFloat()
                }
    ...
    fun setView(
        ...
        if (mRelativeToController != null) {
            ...
            val dpos_y = (
                dry - dradius * cos(angle.toDouble()) +
                    radius * sin(angle.toDouble()) * dangle
                ).toFloat()
            dv_x = dpos_x
            dv_y = dpos_y
            v_x = pos_x
            ...
            if (!path_rotate.isNaN()) {
                val rot = (path_rotate + Math.toDegrees(atan2(dv_y.toDouble(), dv_x.toDouble()))).toFloat()
            ...
            if (!path_rotate.isNaN()) {
                ...
                rot += (path_rotate + Math.toDegrees(atan2(dy.toDouble(), dx.toDouble()))).toFloat()
                view.setRotationZ(rot)
                if (DEBUG) {
    ...
    fun getRect(toUse: IntArray, data: DoubleArray, path: FloatArray, offset: Int) {
        ...
        if (mRelativeToController != null) {
            val rx = mRelativeToController!!.getCenterX()
            val ry = mRelativeToController!!.getCenterY()
            val radius = v_x
            val angle = v_y
            // TODO Debug angle
            v_x = (rx + radius * sin(angle.toDouble()) - v_width / 2).toFloat()
    ...
    fun setDpDt(
        ...
        for (i in toUse.indices) {
            ...
            if (DEBUG) {
                mod += (" , D" + sNames[toUse[i]]).toString() + "/Dt= " + deltaV
            }
            when (toUse[i]) {
                OFF_POSITION -> {}
                OFF_X -> d_x = deltaV
    ...
    override fun compareTo(o: MotionPaths): Int {
        return compare(mPosition, o.mPosition)
    }
    ...
    fun applyParameters(c: MotionWidget) {
        ...
        val at = c.getCustomAttributeNames()
        for (s in at) {
    ...
    fun configureRelativeTo(toOrbit: Motion) {
        @Suppress("unused")
        val p = toOrbit.getPos(mProgress.toDouble()) // get the position
        // in the orbit
    }
