 ...
 */
package androidx.constraintlayout.core.shaded
...
open class ArrayRow : LinearSystem.Row {
    ...
    private var mVariablesToUpdate = ArrayList<SolverVariable>()
    ...
    fun hasKeyVariable(): Boolean {
        return !(mVariable == null || (mVariable!!.mType != SolverVariable.Type.UNRESTRICTED && mConstantValue < 0))
    }
    ...
    fun chooseSubject(system: LinearSystem?): Boolean {
        ...
        val pivotCandidate: SolverVariable? = chooseSubjectInVariables(system)
    ...
    private fun isNew(variable: SolverVariable?, system: LinearSystem?): Boolean {
        if (FULL_NEW_CHECK) {
            var isNew = true
            for (i in 0 until system!!.mNumRows) {
    ...
    override val isEmpty: Boolean
        get() = mVariable == null && mConstantValue == 0f && variables!!.currentSize == 0
    ...
    override fun updateFromRow(system: LinearSystem?, definition: ArrayRow?, removeFromDefinition: Boolean) {
        ...
        if (LinearSystem.SIMPLIFY_SYNONYMS && mVariable != null && variables!!.currentSize == 0) {
            mIsSimpleDefinition = true
    ...
    fun updateFromSynonymVariable(
        ...
        variables!!.add(
            system.mCache.mIndexedVariables.get(variable.mSynonym),
            value,
    ...
    private fun pickPivotInVariables(
        avoid: BooleanArray?,
        exclude: SolverVariable?,
    ): SolverVariable? {
        val all = true
        var value = 0f
    ...
    override fun initFromRow(row: LinearSystem.Row?) {
        if (row is ArrayRow) {
            ...
            for (i in 0 until copiedRow.variables!!.currentSize) {
                val variable = copiedRow.variables!!.getVariable(i)
                val value = copiedRow.variables!!.getVariableValue(i)
                variables!!.add(variable, value, true)
            }
        }
    }
    ...
    override fun addError(error: SolverVariable?) {
        var weight = 1f
        if (error!!.strength == SolverVariable.Companion.STRENGTH_LOW) {
            weight = 1f
        } else if (error.strength == SolverVariable.Companion.STRENGTH_MEDIUM) {
            weight = 1E3f
        } else if (error.strength == SolverVariable.Companion.STRENGTH_HIGH) {
            weight = 1E6f
        } else if (error.strength == SolverVariable.Companion.STRENGTH_HIGHEST) {
            weight = 1E9f
        } else if (error.strength == SolverVariable.Companion.STRENGTH_EQUALITY) {
            weight = 1E12f
        }
        variables!!.put(error, weight)
    }

    override val key: SolverVariable?
        get() = mVariable
    ...
    override fun updateFromSystem(system: LinearSystem?) {
        ...
        while (!done) {
            ...
            val size = mVariablesToUpdate.size
            if (size > 0) {
                for (i in 0 until size) {
                    val variable = mVariablesToUpdate[i]
